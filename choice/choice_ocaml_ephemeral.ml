...
let rec mlist_map f = function
  | Nil -> Nil
  | Cons { hd; tl } ->
    let dst = Cons { hd = f hd; tl = Nil } in
...
let return x = fresh (Cons { hd = x; tl = Nil })
...
let pair l1 l2 =
  ...
  and pair2 acc x1 xs1 xs2 l2 =
    ...
    | Cons { hd = x2; tl = xs2 } -> pair2 (Cons { hd = (x1, x2); tl = acc }) x1 xs1 xs2 l2
  in
  fresh (pair1 Nil l1 l2)

let bind f li =
  let rec flatten_rev_map f acc = function
    | Nil -> acc
    | Cons { hd; tl } ->
      let li = consume (f hd) in
